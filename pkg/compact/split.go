...
func (c SplitConfig) ShardCount(metas []*metadata.Meta, sourceShards, floor uint64) (count uint64, fits bool) {
	...
	fits = nextPow2(byIndex)*lineage <= c.effectiveMaxShards()
...
// the stream has.
func (p *splitPlanner) decide(groupLabels map[string]string, resolution int64, sources []*metadata.Meta) (pieces []ShardRef, count uint64, fits bool, err error) {
...
func (p *splitPlanner) PlanOutputs(_ context.Context, cg *Group, sources []*metadata.Meta) ([]PlanOutput, error) {
	if len(sources) == 0 {
		return nil, nil
	}
	groupLabels := cg.Labels().Map()
	pieces, count, fits, err := p.decide(groupLabels, cg.Resolution(), sources)
	if err != nil {
		return nil, err
	}
	if !fits {
	...
	for _, piece := range pieces {
		...
		outputs = append(outputs, PlanOutput{Labels: lbls, Series: &SeriesPartition{Index: piece.Index, Count: piece.Count}})
...
func (p *splitPlanner) Plan(ctx context.Context, metasByMinTime []*metadata.Meta, errChan chan error, extensions any) ([]*metadata.Meta, error) {
	...
	for range len(metasByMinTime) + 1 {
		...
		_, count, fits, err := p.decide(plan[0].Thanos.Labels, plan[0].Thanos.Downsample.Resolution, plan)
		if err != nil {
			return nil, err
		}
		if fits {
