...
// TestPlanWaitsForSiblingNestedInsideThePreviousFence pins down that "last
// block standing" follows the planner's MinTime ordering: an unmarked block
// nested inside the previous fence sorts after that fence and forms a merge
// run with the fenced block, although it ends before the fence window starts.
// Waiving the fenced block would leave its early downsample behind after the
// merge.
func TestPlanWaitsForSiblingNestedInsideThePreviousFence(t *testing.T) {
	left := ulid.MustNew(1, nil)
	nested := ulid.MustNew(2, nil)
	mid := ulid.MustNew(3, nil)
	right := ulid.MustNew(4, nil)
	newest := ulid.MustNew(5, nil)
	metas := map[ulid.ULID]*metadata.Meta{
		left:   planMeta(left, ResLevel0, 0, 800),
		nested: planMeta(nested, ResLevel0, 200, 400), // A late replica of a range left already spans.
		mid:    planMeta(mid, ResLevel0, 800, 1000),
		right:  planMeta(right, ResLevel0, 1200, 1400),
		newest: planMeta(newest, ResLevel0, 1400, 1600),
	}
	marks := map[ulid.ULID]*metadata.NoCompactMark{
		left:  noCompactMark(left, metadata.IndexSizeExceedingNoCompactReason),
		right: noCompactMark(right, metadata.IndexSizeExceedingNoCompactReason),
	...
	testutil.Assert(t, !planned(got, left), "a fence with an overlapping sibling is not final either")
	testutil.Assert(t, planned(got, right), "the untouched fence is final")
...
func TestPlanStuckBlockLateArrival(t *testing.T) {
	for _, res := range []int64{ResLevel0, ResLevel1} {
		t.Run(fmt.Sprint(res), func(t *testing.T) {
			left, mid, right := ulid.MustNew(1, nil), ulid.MustNew(2, nil), ulid.MustNew(3, nil)
			first, late, merged, replacement := ulid.MustNew(4, nil), ulid.MustNew(5, nil), ulid.MustNew(6, nil), ulid.MustNew(7, nil)
			target := ResLevel1
			if res == ResLevel1 {
				target = ResLevel2
			}
			metas := map[ulid.ULID]*metadata.Meta{
				left: planMeta(left, res, 0, 100), mid: planMeta(mid, res, 200, 300), right: planMeta(right, res, 400, 500),
			}
			marks := map[ulid.ULID]*metadata.NoCompactMark{
				left:  noCompactMark(left, metadata.IndexSizeExceedingNoCompactReason),
				right: noCompactMark(right, metadata.IndexSizeExceedingNoCompactReason),
...
func TestPlanScopesCoverageByExternalLabels(t *testing.T) {
	...
	metas := map[ulid.ULID]*metadata.Meta{
		raw1:  shard(planMeta(raw1, ResLevel0, 0, ResLevel1DownsampleRange, source), "1_of_2"),
		raw2:  shard(planMeta(raw2, ResLevel0, 0, ResLevel1DownsampleRange, source), "2_of_2"),
