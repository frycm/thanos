...
func submitTask(t *testing.T, s *Scheduler, id string) <-chan Result {
	t.Helper()
	ch, err := s.Submit(context.Background(), Task{ID: id, Type: TaskCompaction, SourceBlocks: []string{"a"}})
	testutil.Ok(t, err)
	return ch
}
...
func TestSchedulerAbortedResultDoesNotCountAsAttempt(t *testing.T) {
	...
	// The requeue backs off before the task is leaseable again (pinned down in
	// TestAbortedRequeueBacksOff); skip it here, this test is about attempts.
	s.mtx.Lock()
