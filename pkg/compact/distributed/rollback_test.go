...
func TestRollbackPlanScopedToJournal(t *testing.T) {
	...
	testutil.Equals(t, 0, len(r.Kept))
}
...
// TestRollbackRefusesUnrecoverableSources asserts a produced block is not
// deleted when what it replaced cannot come back: a source that was
// physically deleted after the delete delay, or one marked by retention,
// which the rollback does not touch. Deleting the block would lose its data
// for good, so the plan refuses and names the block, unless told to keep the
// block (nothing lost, the standalone compactor takes it on) or to delete it
// anyway.
func TestRollbackRefusesUnrecoverableSources(t *testing.T) {
	ctx := context.Background()

	t.Run("source physically deleted", func(t *testing.T) {
		f := newRollbackFixture(t)
		f.remove(t, f.sourceA)

		_, err := PlanRollback(ctx, log.NewNopLogger(), f.bkt, RollbackOptions{JournalID: "A"})
		testutil.NotOk(t, err)
		testutil.Assert(t, strings.Contains(err.Error(), f.producedA.String()), "the refusal must name the block, got: %v", err)
		testutil.Assert(t, strings.Contains(err.Error(), f.sourceA.String()), "the refusal must name the source, got: %v", err)

		// Kept: producedA stays, but consumedA still goes, since its own source
		// is restorable; producedA's manager mark is restored so it stays visible.
		r, err := PlanRollback(ctx, log.NewNopLogger(), f.bkt, RollbackOptions{JournalID: "A", Unrecoverable: KeepUnrecoverable})
		testutil.Ok(t, err)
		testutil.Equals(t, []ulid.ULID{f.producedA}, r.Kept)
		testutil.Equals(t, []ulid.ULID{f.consumedA}, r.Produced)
		testutil.Equals(t, []ulid.ULID{f.rawA}, r.Restore)
		testutil.Ok(t, r.Apply(ctx, log.NewNopLogger(), f.bkt))
		testutil.Equals(t, true, exists(t, f.bkt, path.Join(f.producedA.String(), block.MetaFilename)))
		testutil.Equals(t, false, exists(t, f.bkt, path.Join(f.consumedA.String(), block.MetaFilename)))
	})

	t.Run("source marked by retention", func(t *testing.T) {
		f := newRollbackFixture(t)
		testutil.Ok(t, block.RemoveMark(ctx, log.NewNopLogger(), f.bkt, f.sourceA,
			prometheus.NewCounter(prometheus.CounterOpts{Name: "test"}), metadata.DeletionMarkFilename))
		f.mark(t, f.sourceA, "block exceeding retention of 24h")

		_, err := PlanRollback(ctx, log.NewNopLogger(), f.bkt, RollbackOptions{JournalID: "A"})
		testutil.NotOk(t, err)
		testutil.Assert(t, strings.Contains(err.Error(), "retention"), "the refusal must say why, got: %v", err)

		r, err := PlanRollback(ctx, log.NewNopLogger(), f.bkt, RollbackOptions{JournalID: "A", Unrecoverable: DeleteUnrecoverable})
		testutil.Ok(t, err)
		testutil.Equals(t, []ulid.ULID{f.producedA}, r.Lost)
		testutil.Equals(t, []ulid.ULID{f.producedA, f.consumedA}, r.Produced)
		// Retention's mark is not ours to remove.
		testutil.Equals(t, []ulid.ULID{f.rawA}, r.Restore)
	})

	t.Run("a kept block is a source that has to pass too", func(t *testing.T) {
		// consumedA's source is gone, so consumedA is kept - and it carries the
		// manager's mark, which the rollback removes, so producedA can still go.
		f := newRollbackFixture(t)
		f.remove(t, f.rawA)

		r, err := PlanRollback(ctx, log.NewNopLogger(), f.bkt, RollbackOptions{JournalID: "A", Unrecoverable: KeepUnrecoverable})
		testutil.Ok(t, err)
		testutil.Equals(t, []ulid.ULID{f.consumedA}, r.Kept)
		testutil.Equals(t, []ulid.ULID{f.producedA}, r.Produced)
		testutil.Equals(t, []ulid.ULID{f.sourceA, f.consumedA}, r.Restore)

		// Had consumedA been marked by retention instead, producedA would be
		// unrecoverable as well, since deleting it would leave consumedA to go.
		testutil.Ok(t, block.RemoveMark(ctx, log.NewNopLogger(), f.bkt, f.consumedA,
			prometheus.NewCounter(prometheus.CounterOpts{Name: "test"}), metadata.DeletionMarkFilename))
		f.mark(t, f.consumedA, "block exceeding retention of 24h")
		r, err = PlanRollback(ctx, log.NewNopLogger(), f.bkt, RollbackOptions{JournalID: "A", Unrecoverable: KeepUnrecoverable})
		testutil.Ok(t, err)
		testutil.Equals(t, []ulid.ULID{f.producedA, f.consumedA}, r.Kept)
		testutil.Equals(t, 0, len(r.Produced))
	})
}

// TestRollbackIgnoresInheritedProvenance asserts a block the standalone
// compactor or downsampler made from a worker's block - which carries the
// worker's stamp, since extensions are inherited - is not treated as the
// worker's. Deleting it would delete data the distributed compactor never
// produced, and its sources were never marked by the manager.
func TestRollbackIgnoresInheritedProvenance(t *testing.T) {
	f := newRollbackFixture(t)
	ctx := context.Background()

	// A 1h downsample of producedA made by the standalone downsampler: the
