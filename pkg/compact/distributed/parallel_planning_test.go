...
func TestPlanGroupMixedSizePlansDoNotOverlap(t *testing.T) {
	...
	for _, tcase := range []struct {
		...
		{name: "raw", resolution: 0},
		{name: "5m", resolution: 300000},
		{name: "1h", resolution: 3600000},
	} {
		for _, vertical := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s/vertical=%t", tcase.name, vertical), func(t *testing.T) {
				...
				for i, tr := range [][2]int64{{0, 8}, {8, 10}, {10, 12}, {12, 14}, {14, 16}, {16, 24}, {48, 50}, {50, 52}} {
					...
					m.Thanos.Downsample.Resolution = tcase.resolution
				...
				plans := e.planGroup(t, cg, first)
				// The next plan would enclose [8h,16h).
...
func TestPlanGroupDispatchesTimeDisjointPlans(t *testing.T) {
	...
	first := firstPlan.Sources
	testutil.Ok(t, err)
	plans := NewRemotePlanExecutor(logger, bkt, testScheduler(t, bkt, ManagerConfig{}), planner, 4, nil).planGroup(t, cg, first)
