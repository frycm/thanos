...
// crash kills the worker the hard way: nothing is reported, as after an OOM
// kill. A plain cancel() is a graceful shutdown, which reports an abort.
func (w *testWorker) crash() {
	w.dead.Store(true)
	w.cancel()
}

// crashableClient drops a crashed worker's reports, so a scenario can tell a
// hard crash (silence, lease expiry) from a graceful shutdown (an abort).
...
func (c crashableClient) Report(ctx context.Context, res Result) error {
	if c.dead.Load() {
		return nil // The report died with the process.
	}
...
func (c *testCluster) newScheduler() *Scheduler {
	c.t.Helper()
	sched, err := NewScheduler(context.Background(), c.logger, c.manager, prometheus.NewRegistry(), ManagerConfig{
		JournalID:   journalID,
...
func (c *testCluster) execute(cg *compact.Group, toCompact []*metadata.Meta) <-chan executeOutcome {
	return c.executeOpts(cg, toCompact, false, nil)
}
...
func (c *testCluster) executeOpts(cg *compact.Group, toCompact []*metadata.Meta, overlapping bool, dedupReplicaLabels []string) <-chan executeOutcome {
	e := NewRemotePlanExecutor(c.logger, c.manager, c.sched, nil, 1, dedupReplicaLabels, nil)
...
func hasMetric(m *dto.Metric, value string) bool {
	for _, l := range m.GetLabel() {
		if l.GetValue() == value {
			return true
		}
...
func TestInteractionPenaltyDedup(t *testing.T) {
	...
	outcome := c.executeOpts(cg, toCompact, true, []string{"receiver_replica"})
...
func TestInteractionDownsampleDedup(t *testing.T) {
	...
	downsamples := prometheus.NewCounterVec(prometheus.CounterOpts{Name: "test_downsamples"}, []string{"resolution"})
	testutil.Ok(t, DispatchDownsampling(ctx, c.logger, c.manager, c.sched,
		map[ulid.ULID]*metadata.Meta{stripped.ULID: &stripped},
		1, metadata.NoneFunc, 1, false, []string{"receiver_replica"}, downsamples, nil))

	// Manager mode feeds the same per-resolution counters the in-process
	// downsampler does, so failure alerts keep working after a mode flip.
	testutil.Equals(t, 1.0, promtestutil.ToFloat64(downsamples.WithLabelValues(stripped.Thanos.ResolutionString())))
	...
	testutil.Equals(t, entry.Task.ID, prov.TaskID)
	// The stamp names this block and the block it was made from, which is
	// what tells it apart from a copy the standalone downsampler inherits.
