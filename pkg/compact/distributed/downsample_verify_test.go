...
func TestVerifyDownsampledBlock(t *testing.T) {
	...
	outID := ulid.MustNew(9, nil)
	want := Provenance{TaskID: "d1", TaskType: TaskDownsample, JournalID: "shard-a", Generation: 2, Sources: []string{sourceMeta.ULID.String()}}
	...
	valid := func() metadata.Meta {
		...
		ext, err := want.For(outID, want.Sources).Stamp(nil)
		testutil.Ok(t, err)
	...
	otherTask := valid()
	ext, err := Provenance{TaskID: "d2", TaskType: TaskDownsample, JournalID: "shard-a", Generation: 2}.For(outID, want.Sources).Stamp(nil)
	testutil.Ok(t, err)
	otherTask.Thanos.Extensions = ext
	testutil.NotOk(t, verifyDownsampledBlock(&otherTask, candidate, want))

	compactionTask := valid()
	ext, err = Provenance{TaskID: "d1", TaskType: TaskCompaction, JournalID: "shard-a", Generation: 2}.For(outID, want.Sources).Stamp(nil)
	testutil.Ok(t, err)
	compactionTask.Thanos.Extensions = ext
	testutil.NotOk(t, verifyDownsampledBlock(&compactionTask, candidate, want))

	// A 1h downsample of a worker's 5m block, made by the standalone
	// downsampler, inherits the 5m block's stamp whole. It is not the
	// worker's block, and the stamp names another block, so it must not pass.
	inherited := valid()
	inherited.ULID = ulid.MustNew(10, nil)
	testutil.NotOk(t, verifyDownsampledBlock(&inherited, candidate, want))
}
