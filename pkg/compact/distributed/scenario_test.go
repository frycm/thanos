...
// replicas stay separate series, as query-time deduplication expects. The
// corpus also includes OTel collector series replicas and ruler block replicas.
//
...
func chunkSamples(t *testing.T, chk chunkenc.Chunk) []scnSample {
	t.Helper()
	var out []scnSample
	it := chk.Iterator(nil)
	for vt := it.Next(); vt != chunkenc.ValNone; vt = it.Next() {
		...
		case chunkenc.ValHistogram:
			...
			out = append(out, scnSample{t: ts, h: fmt.Sprintf("%#v", h)})
