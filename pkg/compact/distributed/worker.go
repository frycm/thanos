...
func (w *Worker) execute(ctx context.Context, task Task, acknowledged *atomic.Bool) Result {
	...
	return w.completeResult(ctx, res, compIDs)
}
...
func (w *Worker) triageExecutionError(ctx context.Context, res Result, err error, aborted Outcome, acknowledged *atomic.Bool) Result {
	...
	if !acknowledged.Load() {
		// The heartbeat goroutine cancelled the task because the manager no
		// longer recognises our lease. The resulting context error is not a
		// compaction failure - the work was taken away from us mid flight.
		res.Outcome = OutcomeAbortedOwnershipLost
		return res
	}
	if ctx.Err() != nil {
		// The task context is gone but the lease was fine: the worker itself is
		// shutting down. Without this triage the cancellation would fall through
		// ClassifyError - the compaction seam wraps it as a halt error - and a
		// routine rolling restart of a worker would halt the manager.
...
func (w *Worker) completeResult(ctx context.Context, res Result, ids []ulid.ULID) Result {
	...
	for _, id := range ids {
		...
		if err != nil {
			...
			return Result{
				...
				ErrorMessage: errors.Wrapf(err, "confirm the checksum of uploaded result block %s", id).Error(),
			}
		}
