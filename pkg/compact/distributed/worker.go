...
func (w *Worker) execute(ctx context.Context, task Task, acknowledged *atomic.Bool) Result {
	...
	compIDs, err := executor.Execute(ctx, dir, cg, compact.Plan{Sources: toCompact, OverlappingBlocks: task.OverlappingBlocks, Outputs: task.Outputs, Siblings: task.Siblings})
	if err != nil {
		return w.triageExecutionError(ctx, res, err, aborted, acknowledged)
	}
