...
func TestRefillWhileOnePlanIsStillRunning(t *testing.T) {
	...
	first, overlap, err := cg.Plan(t.Context(), executor.planner, make(chan error, 1))
	testutil.Ok(t, err)
	ctx, cancel := context.WithCancel(t.Context())
	done := make(chan struct{})
	go func() { defer close(done); _, _ = executor.Execute(ctx, "", cg, first, overlap) }()
...
func TestParkedPlanDoesNotStarveDisjointWork(t *testing.T) {
	...
	first, overlap, err := cg.Plan(t.Context(), planner, make(chan error, 1))
	testutil.Ok(t, err)
	...
	healthy, _, err := cg.PlanExcluding(t.Context(), planner, exclude, make(chan error, 1))
	testutil.Ok(t, err)
	testutil.Equals(t, 2, len(healthy))
	testutil.Equals(t, 48*h, healthy[0].MinTime)
	ctx, cancel := context.WithCancel(t.Context())
	done := make(chan error, 1)
	go func() { _, err := executor.Execute(ctx, "", cg, first, overlap); done <- err }()
	defer cancel()
	var leased *Task
	deadline := time.Now().Add(5 * time.Second)
	for time.Now().Before(deadline) {
...
func TestGroupExecutionStopsRefillingOnErrorAndPreservesHalt(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(t.Context())
	defer cancel()
	first, overlap, err := cg.Plan(ctx, executor.planner, make(chan error, 1))
	testutil.Ok(t, err)
	done := make(chan error, 1)
	go func() { _, err := executor.Execute(ctx, "", cg, first, overlap); done <- err }()
	var tasks []*Task
