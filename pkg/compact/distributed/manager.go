...
type ManagerConfig struct {
	...
	SelectorHash string
	// DedupFunc and DedupReplicaLabels are the merge configuration
	// (--deduplication.func and --deduplication.replica-label) every worker
	// has to share with the manager. They are stamped on each task and a
	// worker configured differently is refused a lease.
	DedupFunc          string
	...
	// willing to dispatch, in summed source series and summed source index
	// bytes. A plan over either limit is recorded in the journal as oversized
	// and its blocks are parked, instead of being handed to workers that will
	// die trying. Zero disables the corresponding limit.
...
func newManagerMetrics(reg prometheus.Registerer) *managerMetrics {
	...
	return &managerMetrics{
		...
		oversizedTasks: factory.NewCounter(prometheus.CounterOpts{
			...
			Help: "Total number of tasks refused because their expected size exceeds the configured worker capacity.",
		}),
...
type pendingTask struct {
	...
	// notBefore keeps a requeued task out of Lease's reach until it passes,
	// so a task that keeps being aborted backs off instead of churning.
	notBefore time.Time
}
...
type Scheduler struct {
	...
	// persistSeq numbers journal snapshots; guarded by mtx.
	persistSeq uint64

	// persistMtx serializes the journal's bucket I/O. It is never held together
	// with mtx by the same goroutine while doing I/O, so heartbeats and leases
	// are never stalled behind object storage. The fields below it are guarded
	// by it.
...
// abortCapFor bounds how many aborts in a row a task survives before it is
// failed. Aborts do not consume attempts, so without a cap of their own a task
// aborted on every lease would be retried forever. The cap is deliberately
// well above MaxAttempts: a whole fleet restarting mid-task is a handful of
// aborts and must not fail anything.
func abortCapFor(maxAttempts int) int {
	return 3 * maxAttempts
}

// abortRequeueBackoff is how long a task sits out after its n-th abort before
// Lease may hand it out again.
...
// expireLeasesLocked requeues tasks whose worker stopped heartbeating. Any
// transition it makes is persisted immediately: the journal is what a worker's
// pre-upload ownership check reads, so a revocation that only exists in memory
// would let a partitioned worker upload for a task somebody else already holds.
...
// persistLocked writes the journal. The caller holds s.mtx and has already
// applied its state change; the bucket I/O itself runs with s.mtx RELEASED, so
// heartbeats and leases are never stalled behind object storage. On failure the
// caller rolls its change back under the re-acquired lock. That leaves one
// deliberate, narrow trade: a concurrent, newer snapshot that succeeded may
// have persisted the state being rolled back. The journal then briefly
// overstates what happened, which is safe - a worker acting on such an entry
// fails its ownership or heartbeat checks, and the journal converges on the
// next write.
...
// journalSnapshot is the journal marshalled under the state lock, ready to be
// uploaded outside it.
...
// persistSnapshot uploads one snapshot, verifying first that no other manager
// has taken the journal over. Writers queue on persistMtx in snapshot order; a
// snapshot that finds a newer one already landed has nothing left to do,
// because that newer snapshot was marshalled after it and contained its state.
...
// recordPersistFailure books a failed journal write and escalates to a halt
// once the journal has been unwritable for longer than the tolerance window.
// Callers hold persistMtx.
...
// sinceLastPersist reports how long ago the journal last landed in the bucket.
func (s *Scheduler) sinceLastPersist() time.Duration {
	s.persistMtx.Lock()
	defer s.persistMtx.Unlock()
	return time.Since(s.lastPersist)
}
...
func (s *Scheduler) updateQueueMetricsLocked() {
	...
	for id, seen := range s.workerSeen {
		// Forget workers gone for many TTLs, or pod churn grows this forever.
...
// Maintain expires stale leases, refreshes queue metrics, and keeps the
// journal's timestamp fresh. It is meant to be called periodically.
//
...
func (s *Scheduler) maintainLocked(ctx context.Context, unpark []string) error {
	...
	for _, taskID := range unpark {
		...
		level.Info(s.logger).Log("msg", "releasing a parked task on request; its source blocks are planned again", "task", taskID, "state", e.State, "sources", strings.Join(e.Task.SourceBlocks, ","))
		delete(s.journal.Tasks, taskID)
		dirty = true
	}
	s.updateQueueMetricsLocked()

	if !dirty && s.sinceLastPersist() < s.conf.LeaseTTL {
		return nil
	}
	return s.persistLocked(ctx)
}

// unparkRequests lists the tasks operators asked to release.
func (s *Scheduler) unparkRequests(ctx context.Context) ([]string, error) {
	prefix := UnparkPrefix(s.conf.JournalID)
	var ids []string
	err := s.bkt.Iter(ctx, prefix, func(name string) error {
		if id := strings.TrimSuffix(strings.TrimPrefix(name, prefix), objstore.DirDelim); id != "" {
			ids = append(ids, id)
		}
		return nil
	})
...
type RemotePlanExecutor struct {
	...
	// dedupReplicaLabels mirrors --deduplication.replica-label and is carried on
	// every task, so workers can strip block metadata the way the manager did.
	dedupReplicaLabels []string

	// deletableChecker gets the same veto over retiring a verified plan's source
	// blocks that it has over the in-process executor's deletions.
...
func NewRemotePlanExecutor(logger log.Logger, bkt objstore.Bucket, sched *Scheduler, planner compact.Planner, maxInflightPerGroup int, dedupReplicaLabels []string, deletableChecker compact.BlockDeletableChecker) *RemotePlanExecutor {
...
func (e *RemotePlanExecutor) Execute(ctx context.Context, _ string, cg *compact.Group, toCompact []*metadata.Meta, overlappingBlocks bool) ([]ulid.ULID, error) {
	...
	if deferred == len(plans) {
		// Nothing but deferred plans: tell the control loop not to rerun the
		// group, or it would replan the same deferred work forever. A partial
		// deferral keeps the rerun, since the other plans made progress.
...
func (e *RemotePlanExecutor) planGroup(ctx context.Context, cg *compact.Group, toCompact []*metadata.Meta) [][]*metadata.Meta {
	...
	if cg.EnableVerticalCompaction() {
		// With vertical compaction blocks of a group overlap by design, so
		// disjoint sources do not imply disjoint outputs. One plan at a time.
		return plans
	}
...
func (e *RemotePlanExecutor) runPlan(ctx context.Context, cg *compact.Group, toCompact []*metadata.Meta, overlappingBlocks bool) ([]ulid.ULID, error) {
	...
	task.Group.DedupReplicaLabels = e.dedupReplicaLabels
	...
	// Aborted outcomes never reach the submitter: the scheduler requeues them,
	// and past the abort cap it fails the task, so what arrives here is either
	// a completion or a failure.
...
func (e *RemotePlanExecutor) verifyAndFinalize(ctx context.Context, cg *compact.Group, toCompact []*metadata.Meta, res Result, overlappingBlocks bool) ([]ulid.ULID, error) {
	...
	sourceIDs := make([]string, 0, len(toCompact))
	planMinTime, planMaxTime := toCompact[0].MinTime, toCompact[0].MaxTime
	for _, m := range toCompact {
		sourceIDs = append(sourceIDs, m.ULID.String())
	...
	// A rejected result is retried as a whole, and the sources stay. Every
	// output verified to be this task's is then a block nobody will ever
	// account for - and worse than an orphan: it overlaps the sources it was
	// meant to replace, so the next sync would find the group overlapping
	// and halt the manager. Delete what is known to be ours before refusing.
	// Only what is known to be ours: a block that failed the provenance check
	// could be anybody's.
	...
	reject := func(err error) ([]ulid.ULID, error) {
		for _, id := range compIDs {
			level.Warn(e.logger).Log("msg", "deleting a result block of a rejected task result", "task", res.TaskID, "block", id, "reason", err)
			if delErr := block.Delete(ctx, e.logger, e.bkt, id); delErr != nil {
				level.Error(e.logger).Log("msg", "could not delete the result block of a rejected task result; it overlaps the plan's sources", "task", res.TaskID, "block", id, "err", delErr)
			}
		}
		return nil, compact.NewRetryError(err)
	}
	for _, raw := range res.OutputBlocks {
		metaPtr, err := fetchVerifiedMeta(ctx, e.bkt, raw, res.OutputChecksums)
		if err != nil {
			return reject(err)
		}
		meta, id := *metaPtr, metaPtr.ULID
		if err := verifyProvenance(&meta, Provenance{
			TaskID: res.TaskID, TaskType: TaskCompaction, JournalID: e.journalID, Generation: res.Generation, Sources: sourceIDs,
		}); err != nil {
	...
	if len(compIDs) == 0 {
		...
		for _, meta := range toCompact {
			if !e.deletableChecker.CanDelete(cg, meta.ULID) {
				level.Info(e.logger).Log("msg", "the deletable checker vetoed deleting an empty source block", "block", meta.ULID)
				continue
			}
	...
	for _, meta := range toCompact {
		if !e.deletableChecker.CanDelete(cg, meta.ULID) {
			level.Info(e.logger).Log("msg", "the deletable checker vetoed retiring a source block", "block", meta.ULID)
			continue
		}
		if err := block.MarkForDeletion(ctx, e.logger, e.bkt, meta.ULID, DeletionDetails(e.journalID, res.TaskID), cg.BlocksMarkedForDeletion()); err != nil {
...
// fetchVerifiedMeta returns the metadata of a result block a worker reported,
// after verifying that the copy in the bucket matches the checksum the worker
// observed at upload time. It is the shared first half of result verification
// for compaction and downsampling; what the block has to BE is checked by the
// caller, this only proves the reported bytes are what landed.
...
func verifyProvenance(m *metadata.Meta, want Provenance) error {
	...
	case want.Sources != nil && !sameSet(got.Sources, want.Sources):
		// The rollback trusts this list to find what the block replaced.
		return errors.Errorf("records sources %v, the task has %v", got.Sources, want.Sources)
	}
	return nil
}

// describeDedupFunc names a --deduplication.func value in a message, where the
// default is an empty string.
func describeDedupFunc(f string) string {
	if f == "" {
		return "the default merge"
	}
	return f
}

// sameSet reports whether two lists name the same elements, in any order.
...
func CompactionTask(cg *compact.Group, toCompact []*metadata.Meta, overlappingBlocks bool) (Task, error) {
	...
	minTime, maxTime := toCompact[0].MinTime, toCompact[0].MaxTime
	for _, m := range toCompact {
		sources = append(sources, m.ULID.String())
		minTime = min(minTime, m.MinTime)
		maxTime = max(maxTime, m.MaxTime)
	}
