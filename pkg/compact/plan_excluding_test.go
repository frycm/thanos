...
func TestPlanExcludingProducesDisjointPlans(t *testing.T) {
	...
	for {
		next, err := cg.PlanExcluding(ctx, planner, inflight, make(chan error, 1))
		testutil.Ok(t, err)
		plan := next.Sources
		if len(plan) == 0 {
			break
...
func TestPlanExcludingEverythingPlansNothing(t *testing.T) {
	...
	plan, err := cg.PlanExcluding(ctx, planner, all, make(chan error, 1))
	testutil.Ok(t, err)
	testutil.Equals(t, 0, len(plan.Sources))
}
...
func TestPlanExcludingNothingMatchesPlan(t *testing.T) {
	...
	want, err := cg.Plan(ctx, planner, make(chan error, 1))
	testutil.Ok(t, err)

	got, err := cg.PlanExcluding(ctx, planner, nil, make(chan error, 1))
	testutil.Ok(t, err)
