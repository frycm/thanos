...
type queryChunkFailureBucket struct {
	objstore.Bucket
	failChunks atomic.Bool
}

func (b *queryChunkFailureBucket) GetRange(ctx context.Context, name string, off, length int64) (io.ReadCloser, error) {
	if b.failChunks.Load() && strings.Contains(name, "/chunks/") {
		return nil, errors.New("injected fallback chunk read failure")
	}
...
func TestResolutionFilteredStoreRawFallback(t *testing.T) {
	for _, replicaLabel := range []string{"prometheus_replica", "receiver_replica", "otelcol_replica", "ruler_replica"} {
		for _, withDownsample := range []bool{false, true} {
			t.Run(replicaLabel+"/"+map[bool]string{false: "downsampling disabled", true: "partially overlapping genealogy"}[withDownsample], func(t *testing.T) {
				ctx := t.Context()
				logger := log.NewNopLogger()
				dir := t.TempDir()
				fault := &queryChunkFailureBucket{Bucket: objstore.NewInMemBucket()}
				bkt := objstore.WithNoopInstr(fault)
				source := func(n uint64) ulid.ULID { return ulid.MustNew(n, nil) }
				makeBlock := func(name string, value float64, sources []ulid.ULID, replica string) *metadata.Meta {
					...
					for i := range 60 {
						...
						for _, s := range []struct {
							...
							lset := labels.FromStrings("__name__", s.name)
					...
					if replicaLabel == "receiver_replica" || replicaLabel == "ruler_replica" {
						ext[replicaLabel] = replica
					}
					m, err := metadata.InjectThanos(logger, filepath.Join(dir, id.String()), metadata.Thanos{Labels: ext, Source: metadata.TestSource}, nil)
					testutil.Ok(t, err)
					m.Compaction.Sources = sources
					testutil.Ok(t, m.WriteToDir(logger, filepath.Join(dir, id.String())))
					return m
				}
				raw := makeBlock("raw_only", 2, []ulid.ULID{source(1), source(2)}, "A")
				testutil.Ok(t, block.Upload(ctx, logger, bkt, filepath.Join(dir, raw.ULID.String()), metadata.NoneFunc))
				replicaRaw := makeBlock("raw_only", 2, []ulid.ULID{source(4), source(5)}, "B")
				testutil.Ok(t, block.Upload(ctx, logger, bkt, filepath.Join(dir, replicaRaw.ULID.String()), metadata.NoneFunc))
				if withDownsample {
					coarseSource := makeBlock("coarse_only", 3, []ulid.ULID{source(1), source(3)}, "B")
				...
				// A failed read of retained fallback data must fail a strict query,
				// rather than silently dropping the affected series.
				fault.failChunks.Store(true)
				qable := creator(true, []string{"prometheus_replica", "receiver_replica", "otelcol_replica", "ruler_replica"}, nil, 0, false, false, nil, NoopSeriesStatsReporter)
				q, err := engine.NewInstantQuery(ctx, qable, promql.NewPrometheusQueryOpts(false, 5*time.Minute), "sum(shared)", time.Unix(60*60, 0))
				testutil.Ok(t, err)
				result := q.Exec(ctx)
