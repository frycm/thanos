...
func runCompact(
	...
	if conf.mode == compactModeManager && conf.managerLeaseTTL <= 0 {
		// The scheduler quietly falls back to a default, but the maintenance
		// loop runs off the raw flag, and a zero interval panics its ticker.
		return errors.New("--compact.manager.lease-ttl must be greater than zero")
	}
	...
	if conf.mode == compactModeManager {
		...
		planExecutor = distributed.NewRemotePlanExecutor(logger, insBkt, scheduler, planner, conf.managerMaxInflightPerGroup, dedupReplicaLabels, compact.DefaultBlockDeletableChecker{})
	}
...
func (cc *compactConfig) registerFlag(cmd extkingpin.FlagClause) {
	...
	cmd.Flag("compact.manager.max-task-series", "Experimental. Refuse to dispatch a task whose source blocks together hold more series than this. A refused task is recorded in the journal as oversized and its blocks are parked; size the limit to what one worker's memory survives. 0 disables the limit.").
		Default("0").Hidden().Uint64Var(&cc.managerMaxTaskSeries)

	cmd.Flag("compact.manager.max-task-index-size", "Experimental. Refuse to dispatch a task whose source blocks together hold more index bytes than this. A refused task is recorded in the journal as oversized and its blocks are parked; size the limit to what one worker's memory survives. 0 disables the limit.").
