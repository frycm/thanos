...
func TestRegression4960_Deadlock(t *testing.T) {
	...
	testutil.Ok(t, err)
	err = downsampleBucket(ctx, logger, metrics, bkt, metas, downsample.PlanOptions{}, dir, 1, 1, metadata.NoneFunc, false)
...
func TestCleanupDownsampleCacheFolder(t *testing.T) {
	...
	metas, _, err := metaFetcher.Fetch(ctx)
	testutil.Ok(t, err)
	testutil.Ok(t, downsampleBucket(ctx, logger, metrics, bkt, metas, downsample.PlanOptions{}, dir, 1, 1, metadata.NoneFunc, false))
