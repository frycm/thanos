...
func (tbc *bucketRollbackConfig) registerBucketRollbackFlag(cmd extkingpin.FlagClause) *bucketRollbackConfig {
	...
	cmd.Flag("unrecoverable-blocks", "What to do with a block a worker produced whose sources cannot be brought back, because one was physically deleted after --delete-delay or carries a deletion mark the rollback does not remove, such as retention's. "+
		"'refuse' fails the plan and names the blocks. 'keep' leaves them in the bucket: they are ordinary blocks the standalone compactor takes on, so no data is lost, at the price of an incomplete rollback. 'delete' deletes them anyway and loses their data.").
		Default(string(distributed.RefuseUnrecoverable)).EnumVar(&tbc.unrecoverable, string(distributed.RefuseUnrecoverable), string(distributed.KeepUnrecoverable), string(distributed.DeleteUnrecoverable))
	return tbc
}
...
func registerBucketRollbackDistributedCompaction(app extkingpin.AppClause, objStoreConfig *extflag.PathOrContent) {
	cmd := app.Command("rollback-distributed-compaction",
		"Experimental. Undo what the distributed compactor (--compact.mode=manager/worker) did to the bucket: "+
			"delete the blocks its workers produced and remove the deletion marks its manager wrote on the blocks they replaced, "+
...
func runBucketRollback(ctx context.Context, logger log.Logger, bkt objstore.InstrumentedBucket, tbc *bucketRollbackConfig) error {
	plan, err := distributed.PlanRollback(ctx, logger, bkt, distributed.RollbackOptions{
		...
		Unrecoverable:         distributed.UnrecoverablePolicy(tbc.unrecoverable),
	})
	if err != nil {
		return err
	}
	...
	fmt.Fprintf(os.Stdout, "  %d block(s) the produced blocks replaced, to be restored\n", len(plan.Restore))
	for _, id := range plan.Restore {
		fmt.Fprintf(os.Stdout, "    restore %s\n", id)
	}
	if len(plan.Kept) > 0 {
		fmt.Fprintf(os.Stdout, "  %d block(s) produced by workers are kept, as allowed, because what they replaced cannot be brought back\n", len(plan.Kept))
		for _, id := range plan.Kept {
			fmt.Fprintf(os.Stdout, "    keep    %s: %s\n", id, plan.Unrecoverable[id])
		}
	}
	if len(plan.Lost) > 0 {
		fmt.Fprintf(os.Stdout, "  WARNING: %d block(s) produced by workers will be deleted although what they replaced cannot be brought back, as allowed; their data is lost\n", len(plan.Lost))
		for _, id := range plan.Lost {
			fmt.Fprintf(os.Stdout, "    lose    %s: %s\n", id, plan.Unrecoverable[id])
		}
	}
