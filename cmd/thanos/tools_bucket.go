...
func listResLevel() []string {
	return []string{
		...
		prommodel.Duration(time.Duration(downsample.ResLevel1) * time.Millisecond).String(),
		prommodel.Duration(time.Duration(downsample.ResLevel2) * time.Millisecond).String()}
}
