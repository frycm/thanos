...
### Block Splitting by Series

The Prometheus index format cannot exceed 64 GiB. When a compaction is estimated to exceed `--compact.block-max-index-size`, the compactor normally marks the biggest source block with a `no-compact-mark.json` and never compacts or deduplicates it again. For very large block streams this freezes blocks permanently, leaves them below the span downsampling needs, and bounds every operation by the size of one block.

With the experimental, hidden flag `--compact.block-split.max-shards` set above one, such a compaction is instead split by series: the compactor writes `M` output blocks for the same time range, where `M` is the smallest power of two under which every output is estimated to stay within `--compact.block-max-index-size` (with the same headroom the planner applies) and, optionally, `--compact.block-split.max-series`. Shard `i` of `M` holds every series whose stable label hash is congruent to `i-1` modulo `M`, and carries the external label `__compactor_shard__="i_of_M"`. Overlapping replicas are deduplicated inside each shard. Series labels named in `--compact.block-split.ignore-labels` are left out of the hash, so that replicas carrying their replica label inside the series land in the same shard; set it before the first split and never change it, since shards are re-split by the same hash and the compactor halts on a plan whose shards would not together hold every series.

The shards of one plan replace their sources only as a complete set, recorded in every shard's metadata; an incomplete set - a split that failed after some shards were uploaded - supersedes nothing and is withheld from the compactor's own view until the rerun supersedes it. Every plan over shard blocks names the blocks that share a set with its sources, so that those stay published once the sources are gone.

Nothing is configured per stream. Every stream starts unsplit; a plan splits only when its estimate demands it, and a shard group that outgrows the limits later splits again on its own, since a shard of `M` divides exactly into shards of `2M`. A later split of the same stream never chooses fewer shards than the stream already has, unless the maximum has been lowered below that count, and a plan never goes below the shard count of its own sources; a shard group compacts and downsamples its last range although the stream's fresh blocks arrive unsplit elsewhere; and a block left alone in a range that finer shards of its stream already cover - an unsplit window one replica missed, a late upload, a shard block whose group re-split that range meanwhile - is split on its own into the shards that already cover its range rather than staying behind forever. Only when a plan - or a block left alone - would need more shards than the maximum does the compactor fall back to marking the biggest block, as before; the mark's message names the shard count the plan would need.

Because the shard is an external label, every shard is its own compaction group: shards of the same range are never mistaken for overlapping blocks, and adjacent shards of one group compact horizontally like any other blocks. The metadata deduplication filter, in the compactor and in Store Gateway, knows one thing about shards: a shard supersedes the blocks it was made from and the coarser shards of its own lineage, but never a sibling shard of the same plan, although both record the same sources. Deciding what still needs downsampling knows the same: a block counts as downsampled through its own shard, a coarser shard holding it, or all the finer shards that hold its series together. Retention also reaches blocks withheld as unpublished, and block repair and `tools bucket rewrite` keep a copied shard in its set. The compactor's own `--selector.relabel-config` must keep all shards of a stream together, since whether a set is complete is judged on what the compactor sees. Turning splitting off again is safe: shard groups keep compacting within themselves. Store Gateway serves shard blocks like any other blocks and strips `__compactor_shard__` from series, label names and label values, so queriers never see it; deploy Store Gateways with this change before enabling splitting. A `--selector.relabel-config` on `__compactor_shard__` can spread the shards of one stream across several Store Gateways.

Metrics: `thanos_compact_block_splits_total`, `thanos_compact_block_split_shards` and `thanos_compact_block_split_fallbacks_total`. The full design is in [the proposal](../proposals-accepted/202609-compactor-block-splitting.md).
