...
With the experimental, hidden flag `--compact.block-split.max-shards` set above one, such a compaction is instead split by series: the compactor writes `M` output blocks for the same time range, where `M` is the smallest power of two under which every output is estimated to stay within `--compact.block-max-index-size` (with the same headroom the planner applies) and, optionally, `--compact.block-split.max-series`. Shard `i` of `M` holds every series whose stable label hash is congruent to `i-1` modulo `M`, and carries the external label `__compactor_shard__="i_of_M"`. Overlapping replicas are deduplicated inside each shard.
