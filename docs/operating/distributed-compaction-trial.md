...
The manager and its workers have to agree on `--compact.manager.journal-id`, `--deduplication.func` and `--deduplication.replica-label`: the manager refuses to hand a task to a worker configured differently, and says which flag to fix, because a worker merging the sources differently than the manager planned for would leave no trace in the blocks.
...
* `thanos_compact_manager_abandoned_tasks_total` - must stay 0. An abandoned task means a block set repeatedly killed its worker without a report; its source blocks are parked, and the two gauges above sit innocently at zero while the shard makes no progress on them. Every abandonment needs an investigation (`state: "abandoned"` in the journal names the blocks).

  A parked block set - abandoned, or `oversized` for a plan over `--compact.manager.max-task-series` / `--compact.manager.max-task-index-size` - is not planned again until its journal entry ages out of `--compact.manager.journal-retention`, and with `--deduplication.replica-label` set the planner keeps proposing the same set first, so the whole group waits with it. Once the cause is fixed (more worker memory, a raised limit, a no-compact mark on the offending block), release the set without restarting the manager by writing an empty object named after the task ID under the journal's `unpark/` directory:

  ```bash
  # for task 01J... in the journal of shard <shard name>
  echo -n | aws s3 cp - s3://<bucket>/compact-manager/<shard name>/unpark/01J...
  ```

  The manager picks it up on its next maintenance tick (a quarter of the lease TTL), drops the entry, logs it, and removes the marker.
...
* `thanos_compact_manager_tasks_total` by `outcome` - `aborted_*`, `failed_*` and `abandoned` outcomes should be rare and explained.
...
* **Every block a worker produces carries its provenance** in the block's `meta.json` extensions, under `thanos_compact_distributed`: the task, the worker, the journal and the journal generation that produced it, the block's own ID, and the blocks the task consumed. This applies to compacted and downsampled blocks alike. It is in the extensions rather than in `Thanos.Source` on purpose: the consistency delay exempts compactor-sourced blocks, and a new source value would hide worker output from the manager's next sync while its sources were already marked for deletion. The block's own ID is there because extensions are inherited - the standalone compactor keeps the extensions its sources agree on, and downsampling copies them whole - so a block the standalone compactor later makes from a worker's block carries the worker's stamp too; a stamp that names another block does not count.
* **Every deletion mark the manager writes names the manager and the task** in its details: `source of block compacted by distributed compactor; journal <id>; task <id>`. Marks written by the standalone compactor, by retention, or by an operator look different and are never touched by the rollback, with one exception: the `outdated block` mark the compactor's garbage collection writes on a block it finds covered by another. That mark lands on a source when the manager was interrupted between a result becoming visible and marking the sources itself, and the rollback removes it from the sources of the blocks it deletes - safely, because if the source really is covered by a block that stays, garbage collection marks it again.

Together they make `thanos tools bucket rollback-distributed-compaction` precise: it deletes exactly the blocks workers produced and restores exactly the blocks they replaced, and nothing else.

It also makes the tool refuse to lose data. Before deleting a produced block it checks every block the task consumed: it has to be in the bucket still, and either unmarked or carrying a mark the rollback removes. A source that was physically deleted after `--delete-delay`, or that retention or an operator marked, cannot be relied on, and deleting the block made from it would lose that data for good - so the plan fails and names the block and the reason. `--unrecoverable-blocks=keep` leaves such blocks in the bucket instead: they are ordinary blocks the standalone compactor takes on as its own, so nothing is lost, at the price of an incomplete rollback. `--unrecoverable-blocks=delete` deletes them anyway. Downsampled blocks are where this comes up in normal operation: raw retention deletes the raw block a worker downsampled long before the downsampled block expires, and after that the downsampled block cannot be rolled back, only kept.
...
The tool deletes the produced blocks *first*, by their `meta.json` first, so they vanish from every reader's view before the sources return. The order matters: were the sources restored while the produced blocks were still visible, a restarted standalone compactor would see the produced blocks cover them and mark the sources as duplicates all over again. Only then does it remove the manager's marks. Finally, remove the journal at `compact-manager/<shard name>/journal.json` and restart the standalone compactor; it finds the bucket as it left it.

The tool refuses to apply while the journal was written within `--manager-liveness-window` (15m by default; a running manager writes its journal at least once per `--compact.manager.lease-ttl`, even when idle), and it refuses to plan at all while any block's metadata cannot be read, since such a block might be a marked source it could not restore. Both refusals have explicit overrides, `--force` and `--allow-unreadable-blocks`; use them only once you have confirmed by other means that nothing is running and that the unreadable blocks are unrelated. It also refuses to plan while a produced block's sources cannot be brought back, see above; `--unrecoverable-blocks=keep` is the override that loses nothing.
