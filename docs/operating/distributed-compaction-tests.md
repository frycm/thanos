...
|--------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Worker cancellation during metadata/chunk reads, compaction, ownership checks, uploads and checksum reads                                              | `TestWorkerShutdownAtExecutionStages`, graceful restart interaction, `TestWorkerShutdownAbortsAreNotChargedOrBackedOff`                                    |
...
| Invalid compaction result: checksum, provenance, labels, sources, resolution, source/time coverage, empty output, set on a plain plan                  | Verification and finalization tests, `TestClaimOutputsRefusesASetOnAPlainPlan`; no source deletion before validation                                       |
...
| The journal write recording the verdict on a result fails                                                                                              | `TestVerificationCountsOnlyOnceRecorded`: the sources are retired only once the journal holds the verdict                                                  |
| A reported output that is not the task's block, such as a source of the plan                                                                           | `TestMaintenanceRejectsUnverifiedOutputs`: maintenance deletes only a block whose own metadata names the task                                              |
| Manager takeover while a worker of the old generation is mid task or already uploading                                                                 | `TestInteractionManagerTakeoverStopsOldWork`: the old work is discarded; `TestSchedulerTakeoverEndsUnfinishedTasks`: a late upload is deleted              |
| A completed, unverified task older than the journal retention at a takeover                                                                            | `TestTakeoverKeepsUnverifiedOutputsPastTheRetention`: not pruned, so its outputs are rejected rather than published                                        |
| Slow deletion of rejected blocks; a manager that has stopped                                                                                           | `TestMaintenanceDeletesWithoutTheStateLock`, `TestStoppedSchedulerWritesNothing`: no journal write after the stop                                          |
