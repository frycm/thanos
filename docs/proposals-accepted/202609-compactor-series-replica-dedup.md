...
**What a querier sees.** The merger deduplicates each group of overlapping chunks on its own and starts every group afresh. For HA replicas written through the same receivers, a group is one block window, since chunks do not cross blocks. Within a window, the result is exactly what a querier's penalty deduplication returns for that window read alone. Across windows it can differ from a querier reading the whole range, which carries its state on:

* **Replicas scraping in lockstep:** every tie goes to the replica offered first, and the result equals what a querier reading the whole range returns.
* **Replicas scraping at different moments,** more than the algorithm's 5-second initial penalty apart: each window starts with the earlier replica's first sample, then settles on one replica, possibly the later one. A querier reading across windows stays with one replica throughout.
* **After a gap that spans a window boundary,** the next window starts again from the replica whose chunk starts first, where a querier would stay with the other.

Every sample kept is one a replica really has, and a window is never left without data a replica has. This is how today's vertical compaction of external replicas behaves as well. Query-time deduplication has no fixed replica order either: the querier strips replica labels and re-sorts with an unstable sort over the order responses arrived in.
