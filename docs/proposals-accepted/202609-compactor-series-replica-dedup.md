...
**A fixed order for the merge function.** The compactor's penalty merger (`pkg/dedup`) walks the replicas' chunks by time; each group of overlapping chunks is merged into the one that starts first, and the penalty algorithm keeps that base's samples where replicas have one at the same timestamp. Two things made the choice depend on something other than the data. `storage.NewMergeChunkSeriesSet` does not keep a stable order among equal series, so the populator merges its per-source, per-replica series sets itself and offers equal series in a fixed order: by source block - MinTime, then ULID, since sources with the same MinTime, such as two receivers' copies of one range, come in no fixed order - then by the replica labels compared as a label set, a series lacking them first. And the merger's heap ordered chunks by time only, so between chunks with the same time range - replicas scraped in lockstep, or blocks written by the same receiver - the base depended on how the heap had settled; it now gives such ties to the series offered first. For series replica labels that makes the result depend on the sources alone. For external replicas the series still reach the merger through `storage.NewMergeChunkSeriesSet`, whose order among equal series is not fixed, so today's vertical compaction of external replicas keeps depending on it. The scenario suite found this: a deduplicated series switched from one replica to the other in the one window whose other receiver copy was missing, which left raw samples valid but changed the cumulative counter aggregate of the downsampled block.

**What a querier sees.** The merger deduplicates each group of overlapping chunks on its own and starts every group afresh. A group spans about one chunk: 120 samples, half an hour at a 15-second scrape, so a two-hour block holds about four. Within a group, the result is exactly what a querier's penalty deduplication returns for that group read alone. Across groups it can differ from a querier reading the whole range, which carries its state on:

* **Replicas scraping in lockstep without gaps:** every tie goes to the replica offered first, and the result equals what a querier reading the whole range returns.
* **Replicas scraping at different moments,** more than the algorithm's 5-second initial penalty apart: each group starts with the earlier replica's first sample, then settles on one replica, possibly the later one. A querier reading across groups stays with one replica throughout, so the result holds a few more samples, one per group boundary.
* **After a gap in one replica,** in lockstep or not, the next group can start again from the replica the querier switched away from, where a querier stays with the other.

Every sample kept is one a replica really has, and a group is never left without data a replica has. This is how today's vertical compaction of external replicas behaves as well. Query-time deduplication has no fixed replica order either: the querier strips replica labels and re-sorts with an unstable sort over the order responses arrived in.
...
Deduplication by series labels is a policy of the whole stream, like the external replica labels and the merge function, and needs nothing of the planner's bucket-wide view. It is configured on the executor, `LocalPlanExecutor.SeriesReplicaLabels`. With block splitting (frycm/thanos#11), the partitions must leave at least these labels out of their hash - its `--compact.block-split.ignore-labels` - and the populator refuses a partition that hashes one of them.

### Read path

The querier keeps its replica labels. Old blocks carry both replicas; deduplicated blocks carry one series without the label; a querier deduplicating by the same labels serves the same series from both. Within each group of overlapping chunks the samples are the ones it served before; at group boundaries the compacted blocks may hold the other replica's samples, as described above. Selectors and `label_values()` on the replica label return nothing for deduplicated ranges, and rules grouping by it see one series where they saw two; that is inherent to removing the label.
...
* Unit tests: the merger's tie-break by series position, whatever earlier chunks did to its heap; the order counter-example; replicas across two sources with the tie going to the first source's first replica, the same whichever order the sources are handed over in; a series without the label merging with its replica; the label leaving the symbol table; replicas with a gap, where penalty deduplication switches replica, matching the querier's penalty deduplication of the same series; partitions composing into the whole output and a partition hashing the replica label refused; the executor recording the labels and counting series.
* Unit tests at normal scrape density, several chunk groups per block, which compute the groups from the source chunks and pin down each group equalling a querier reading it alone, and the difference from a querier reading the whole block for offset scrapes and for a gap in lockstep replicas; one of a gap across blocks; and of the comparison the scenarios use, which rejects output alternating between healthy replicas, the union of offset scrapes, a lost and a duplicated sample.
* Scenario suite: the generic fault scenarios - object store outage, process crash, corrupted source, failed publications of every block file - with deduplication on, judged against the standalone compactor with it on. Two scenarios judge the result against a standalone run without deduplication through the querier's own penalty deduplicator, comparing whole sequences over every replica order. With replicas in lockstep the whole range is compared, downsampled aggregates included; with replicas scraping 20 seconds apart, raw samples are compared window by window, each window one chunk group in that corpus.
* The command starts with penalty deduplication by a series replica label alone, without an external replica label.
